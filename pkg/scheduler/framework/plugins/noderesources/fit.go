...
func (f *Fit) Filter(ctx context.Context, cycleState *framework.CycleState, pod *v1.Pod, nodeInfo *framework.NodeInfo) *framework.Status {
	if !f.enableSidecarContainers && hasSidecarContainer(pod) {
		// To avoid the inconsistency in resource calculation between the scheduler
		// and the older (before v1.28) kubelet
		return framework.NewStatus(framework.UnschedulableAndUnresolvable, "SidecarContainers feature is disabled")
	}
