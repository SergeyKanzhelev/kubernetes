...
const (
	...
	reasonKillPod             containerKillReason = "KillPod"
...
func (m *kubeGenericRuntimeManager) computePodActions(ctx context.Context, pod *v1.Pod, podStatus *kubecontainer.PodStatus) podActions {
	...
	if killPod && len(changes.ContainersToKill) == 0 {
		// All regular containers completed and no sidecar containers to kill.
		// We can kill the pod now.
