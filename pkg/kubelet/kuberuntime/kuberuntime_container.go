...
func (m *kubeGenericRuntimeManager) computeInitContainerActions(pod *v1.Pod, podStatus *kubecontainer.PodStatus, changes *podActions) bool {
	...
	previousContainerInitialized := true
	...
	for i := range pod.Spec.InitContainers {
		...
		if status == nil {
			if podHasInitialized || previousContainerInitialized {
				// For the new init container,
				// - if the pod has been initialized before, we should restart the
				// sidecar container here as it should restart always.
				// - if the previous init container has been initialized, it is time to
				// start the next one.
		...
		previousContainerInitialized = false
		switch status.State {
	...
	if previousContainerInitialized {
		podHasInitialized = true
	}
