...
func (m *ManagerImpl) HandleWatchResourcesStream(ctx context.Context, stream drapb.Node_WatchResourcesClient, resourceName string) {

	// run this in a goroutine - one per Recv (per driver).
	go func() {
		logger := klog.FromContext(ctx)
		...
		for {
			select {
			case <-ctx.Done():
				logger.Info("Stop watching resources health")
				// TODO (SergeyKanzhelev): remove all health information for this plugin
				return
			default:
				msg, err := stream.Recv()
				if err == io.EOF {
					logger.Info("DRA plugin stopped reporting resources health")
					// TODO (SergeyKanzhelev): remove all health information for this plugin
					return
				} else if err != nil {
					if status.Code(err) == codes.Canceled {
						// We expect the caller to clean up all health information for this plugin's devices
						logger.Info("Stop watching resources health")
						return
					}
					// ignore any other errors
					logger.Error(err, "Error watching resources health, ignoring")
				}
				// Process the received message
				err = m.pluginWatchResourcesReceiver(ctx, resourceName, msg)
				if err != nil {
					logger.Error(err, "Error updating resources health")
				}
			}
		}
	}()
...
func (m *ManagerImpl) pluginWatchResourcesReceiver(ctx context.Context, resourceName string, resp *drapb.WatchResourcesResponse) error {
	if utilfeature.DefaultFeatureGate.Enabled(features.ResourceHealthStatus) {
		...
		klog.V(5).InfoS("Device health info updated", "resourceName", resourceName, "updated", updated, "podsToUpdate", podsToUpdate.UnsortedList())
...
func (m *ManagerImpl) UpdateAllocatedResourcesStatus(pod *v1.Pod, status *v1.PodStatus) {
	for _, c := range m.cache.claimInfo {
		if c.PodUIDs.Has(string(pod.UID)) {
			...
			// this is a pod's claim name
			var claimName string
			for _, rc := range pod.Spec.ResourceClaims {
				if *rc.ResourceClaimName == c.ClaimName {
					claimName = rc.Name
				}
			}

			klog.InfoS("UpdateAllocatedResourcesStatus", "claim", c.ClaimName, "claimName", claimName, "pod", klog.KObj(pod))
			...
			if !found {
				...
				klog.Warningf("Claim %s (%s) not found in any container of pod %s/%s", c.ClaimName, claimName, pod.Namespace, pod.Name)
			}
		}
	}
}
